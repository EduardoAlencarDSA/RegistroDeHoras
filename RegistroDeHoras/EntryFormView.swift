    ...
    var body: some View {
        Form {
            ...
            Section("HORÁRIO") {
                ...
                // cálculo do total líquido com base na data selecionada
                let baseDay = Calendar.current.startOfDay(for: date)
                let lunchStart = Calendar.current.date(bySettingHour: 12, minute: 0, second: 0, of: baseDay)!
                let lunchEnd   = Calendar.current.date(bySettingHour: 13, minute: 0, second: 0, of: baseDay)!

                let start = date.combiningTime(from: startTime)
                let end   = date.combiningTime(from: endTime)

                let bruto = max(0, end.timeIntervalSince(start) / 3600.0)

                // interseção com almoço
                let overlapStart = max(start, lunchStart)
                let overlapEnd   = min(end, lunchEnd)
                let lunchOverlap = max(0, overlapEnd.timeIntervalSince(overlapStart)) / 3600.0

                let liquido = max(0, bruto - lunchOverlap)
